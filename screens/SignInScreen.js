...
const SignInScreen = ({navigation}) => {
  ...
  const loginHandle = (userName, password) => {
    ...
    const foundUser = user_test.filter(item => {
      ...
      return (
        userName == item.fields.username && password == item.fields.password
      );
    });
    if (data.username.length == 0 || data.password.length == 0) {
      Alert.alert('Erro!', 'Campo do usuário ou senha está em branco.', [
        {text: 'Ok'},
      ]);
      return;
    }
    ...
    if (foundUser.length == 0) {
      Alert.alert('Dados inválidos!', 'Usuário ou senha está incorreto.', [
        {text: 'Ok!'},
      ]);
      return;
    }
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.header}>
        <Text style={styles.text_header}>Seus dados</Text>
      </View>
      <Animatable.View
